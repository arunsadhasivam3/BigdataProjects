...
imputeMissingValue<-function(df){
  ...
  df$CoapplicantIncome <- as.numeric(as.character( df$CoapplicantIncome))
  df$ApplicantIncome <- as.integer(as.character( df$ApplicantIncome))
  df$Loan_Amount_Term <- as.integer(as.character( df$Loan_Amount_Term))
  df$LoanAmount   <- as.integer(as.character( df$LoanAmount  ))
  ...
  #as.numeric(as.character(df$ApplicantIncome  )) return correct value
  #as.numeric(df$ApplicantIncome  ) return factor wrong value it takes value
...
predictOutput <- function (){
  ...
  test<-dplyr::select(test,1:Loan_Status)
  ...
  return (persistData(test)) ## important to return then only in ui it can get and return in rshiny
}
  ...
  for( i in 1:nrow(test)){  
    ...
    for( j in 1:rowLength){
      ...
      if(class(result)=='factor'){
        cellvalue<-levels(drop.levels(result) )
        mongo.bson.buffer.append(buf1, key, cellvalue)
      }else if (typeof(result)=='double'){
        cellvalue<- as.double(result)
        mongo.bson.buffer.append.double(buf1, key, cellvalue)
      }else if (typeof(result)=='integer'){
        cellvalue<-as.integer(result)
        mongo.bson.buffer.append(buf1, key, cellvalue)
      }
    }
